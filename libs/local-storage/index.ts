...
export class LocalStorage {
  private static readonly logger = new Logger("LocalStorage");
  ...
  static async setObject<T>(key: LocalStorageKey, value: T): Promise<void> {
    try {
      ...
      this.logger.debug(`Set object for key "${key}"`);
    } catch (error) {
      this.logger.exception(
        error as Error,
  ...
  static async getObject<T>(key: LocalStorageKey): Promise<T | null> {
    try {
      ...
      if (jsonValue) {
        this.logger.debug(`Retrieved object for key "${key}"`);
        return JSON.parse(jsonValue) as T;
      } else {
        this.logger.debug(`No object found for key "${key}"`);
        return null;
      }
    } catch (error) {
      this.logger.exception(
