...
export class GeolocationService {
  ...
  static async requestPermissions(): Promise<Location.PermissionResponse> {
    try {
      ...
      logger.debug(`Permission status: ${permission.status}`);
      return permission;
    } catch (error) {
  ...
  static async hasPermissions(): Promise<boolean> {
    try {
      ...
      logger.exception(error as Error, "Failed to check permissions");
      return false;
    }
  }
  ...
  static async getCurrentPosition(
    ...
    try {
      ...
      logger.exception(error as Error, "Failed to get current position");
      throw error;
    }
  }
  ...
  static async getLastKnownPosition(
    ...
    try {
      ...
      if (cached && this.isCacheValid(cached, maxAge)) {
        logger.debug("Returning cached position");
      ...
      const hasPermission = await this.hasPermissions();
      if (!hasPermission) {
        logger.debug("No permissions, returning null");
        return null;
      }

      const location = await Location.getLastKnownPositionAsync();
      if (!location) {
        logger.debug("No last known position available");
        return null;
      }
      ...
      logger.exception(error as Error, "Failed to get last known position");
      return null;
    }
  }
  ...
  static async cachePosition(position: GeolocationPosition): Promise<void> {
    try {
      ...
      logger.exception(error as Error, "Failed to cache position");
  ...
  static async getCachedPosition(): Promise<CachedGeolocation | null> {
    try {
      ...
      logger.debug("Retrieved cached position");
      return parsed;
    } catch (error) {
      logger.exception(error as Error, "Failed to get cached position");
      return null;
    }
  }
  ...
  static async clearCache(): Promise<void> {
    try {
      ...
      logger.exception(error as Error, "Failed to clear cache");
    }
  }
  ...
  private static async cacheGeocodedAddress(
    ...
    try {
      ...
      logger.debug(`Cached geocoded address for key ${coordinateKey}`);
    } catch (error) {
      logger.exception(error as Error, "Failed to cache geocoded address");
      // Don't throw - caching failure shouldn't break the app
    }
  }
  ...
  private static async getCachedGeocodedAddress(
    ...
    try {
      ...
      if (!entry) {
        logger.debug(`No cached address found for key ${coordinateKey}`);
        return null;
      }
      ...
      if (age > this.GEOCODE_CACHE_EXPIRY_MS) {
        logger.debug(`Cached address expired for key ${coordinateKey}`);
        return null;
      }

      logger.debug(`Retrieved cached address for key ${coordinateKey}`);
      return entry.addresses as Location.LocationGeocodedAddress[];
    } catch (error) {
      logger.exception(error as Error, "Failed to get cached geocoded address");
      return null;
    }
  }
  ...
  static async clearGeocodedAddressCache(): Promise<void> {
    try {
      ...
      logger.debug("Geocoded address cache cleared");
    } catch (error) {
      logger.exception(
        error as Error,
  ...
  static async reverseGeocode(
    ...
    try {
      ...
      if (cached) {
        logger.debug("Returning cached geocoded address");
        return cached;
      }

      // Cache miss - call API
      logger.debug("Cache miss, calling reverse geocoding API");
      const addresses = await Location.reverseGeocodeAsync({
        latitude: coords.latitude,
        longitude: coords.longitude,
      });

      logger.debug(`Found ${addresses.length} addresses`);
      ...
      logger.exception(error as Error, "Failed to reverse geocode");
      throw error;
    }
  }
  ...
  static async saveLocation(
    ...
    try {
      logger.debug(`Saving location "${name}"`);
      ...
      logger.debug(`Location "${name}" saved successfully`);
      return savedLocation;
    } catch (error) {
      logger.exception(error as Error, `Failed to save location "${name}"`);
      throw error;
    }
  }
  ...
  static async getSavedLocations(): Promise<SavedLocation[]> {
    try {
      ...
      logger.debug(`Retrieved ${locations.length} saved locations`);
      return locations;
    } catch (error) {
      logger.exception(error as Error, "Failed to get saved locations");
      return [];
    }
  }
  ...
  static async getSavedLocationById(id: string): Promise<SavedLocation | null> {
    try {
      ...
      logger.exception(error as Error, `Failed to get location by ID "${id}"`);
      return null;
    }
  }
  ...
  static async getSavedLocationByName(
    ...
    try {
      ...
      if (location) {
        logger.debug(`Found location with name "${name}"`);
      } else {
        logger.debug(`No location found with name "${name}"`);
      }
  ...
  static async updateSavedLocation(
    ...
    try {
      ...
      if (index === -1) {
        logger.debug(`Location with ID "${id}" not found`);
        return null;
      }
  ...
  static async clearSavedLocations(): Promise<void> {
    try {
      ...
      logger.debug("All saved locations cleared");
    } catch (error) {
      logger.exception(error as Error, "Failed to clear saved locations");
    }
  }
  ...
  static async findLocationsNearby(
    ...
    try {
      ...
      logger.debug(`Found ${nearby.length} locations within ${radiusMeters}m`);
      return nearby;
    } catch (error) {
      logger.exception(error as Error, "Failed to find nearby locations");
      return [];
    }
  }
