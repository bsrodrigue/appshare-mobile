...
export class GeolocationService {
    ...
    static async requestPermissions(): Promise<Location.PermissionResponse> {
        try {
            Logger.debug('GeolocationService: Requesting location permissions');
            const permission = await Location.requestForegroundPermissionsAsync();
            Logger.debug(`GeolocationService: Permission status: ${permission.status}`);
            return permission;
        } catch (error) {
            Logger.exception(error as Error, 'GeolocationService: Failed to request permissions');
            throw error;
        }
    }
    ...
    static async hasPermissions(): Promise<boolean> {
        try {
            ...
            Logger.exception(error as Error, 'GeolocationService: Failed to check permissions');
            return false;
        }
    }
    ...
    static async getCurrentPosition(
        ...
        try {
            Logger.debug('GeolocationService: Getting current position');
            ...
            Logger.debug(
                `GeolocationService: Got position: ${position.coords.latitude}, ${position.coords.longitude}`
            );
            ...
            Logger.exception(error as Error, 'GeolocationService: Failed to get current position');
            throw error;
        }
    }
    ...
    static async getLastKnownPosition(
        ...
        try {
            Logger.debug('GeolocationService: Getting last known position');

            // Try to get from cache first
            const cached = await this.getCachedPosition();
            if (cached && this.isCacheValid(cached, maxAge)) {
                Logger.debug('GeolocationService: Returning cached position');
            ...
            const hasPermission = await this.hasPermissions();
            if (!hasPermission) {
                Logger.debug('GeolocationService: No permissions, returning null');
                return null;
            }

            const location = await Location.getLastKnownPositionAsync();
            if (!location) {
                Logger.debug('GeolocationService: No last known position available');
                return null;
            }
            ...
            Logger.exception(error as Error, 'GeolocationService: Failed to get last known position');
            return null;
        }
    }
    ...
    static async cachePosition(position: GeolocationPosition): Promise<void> {
        try {
            ...
            Logger.debug('GeolocationService: Position cached successfully');
        } catch (error) {
            Logger.exception(error as Error, 'GeolocationService: Failed to cache position');
    ...
    static async getCachedPosition(): Promise<CachedGeolocation | null> {
        try {
            const cached = await SecureStorage.getItem(SecureStorageKey.CACHED_GEOLOCATION);
            if (!cached) {
                return null;
            }

            const parsed: CachedGeolocation = JSON.parse(cached);
            Logger.debug('GeolocationService: Retrieved cached position');
            return parsed;
        } catch (error) {
            Logger.exception(error as Error, 'GeolocationService: Failed to get cached position');
            return null;
        }
    }
    ...
    static async clearCache(): Promise<void> {
        try {
            ...
            Logger.debug('GeolocationService: Cache cleared');
        } catch (error) {
            Logger.exception(error as Error, 'GeolocationService: Failed to clear cache');
        }
    }
     ...
     */
    private static isCacheValid(cached: CachedGeolocation, maxAge: number): boolean {
    ...
    private static async cacheGeocodedAddress(
        ...
        try {
            ...
            // Get existing cache
            const cacheData = await SecureStorage.getItem(SecureStorageKey.CACHED_GEOCODED_ADDRESSES);
            const cache: Record<string, CachedGeocodedAddress> = cacheData ? JSON.parse(cacheData) : {};
            ...
            Logger.debug(`GeolocationService: Cached geocoded address for key ${coordinateKey}`);
        } catch (error) {
            Logger.exception(error as Error, 'GeolocationService: Failed to cache geocoded address');
            // Don't throw - caching failure shouldn't break the app
        }
    }
    ...
    private static async getCachedGeocodedAddress(
        ...
        try {
            ...
            const cache: Record<string, CachedGeocodedAddress> = JSON.parse(cacheData);
            const entry = cache[coordinateKey];

            if (!entry) {
                Logger.debug(`GeolocationService: No cached address found for key ${coordinateKey}`);
                return null;
            }
            ...
            if (age > this.GEOCODE_CACHE_EXPIRY_MS) {
                Logger.debug(`GeolocationService: Cached address expired for key ${coordinateKey}`);
                return null;
            }

            Logger.debug(`GeolocationService: Retrieved cached address for key ${coordinateKey}`);
            return entry.addresses as Location.LocationGeocodedAddress[];
        } catch (error) {
            Logger.exception(error as Error, 'GeolocationService: Failed to get cached geocoded address');
            return null;
        }
    }
    ...
    static async clearGeocodedAddressCache(): Promise<void> {
        try {
            await SecureStorage.removeItem(SecureStorageKey.CACHED_GEOCODED_ADDRESSES);
            Logger.debug('GeolocationService: Geocoded address cache cleared');
        } catch (error) {
            Logger.exception(error as Error, 'GeolocationService: Failed to clear geocoded address cache');
        }
    }
     ...
     */
    static async reverseGeocode(coords: Coordinates): Promise<Location.LocationGeocodedAddress[]> {
        try {
            Logger.debug(`GeolocationService: Reverse geocoding coordinates: ${coords.latitude}, ${coords.longitude}`);

            // Check cache first
            const cached = await this.getCachedGeocodedAddress(coords);
            if (cached) {
                Logger.debug('GeolocationService: Returning cached geocoded address');
                return cached;
            }

            // Cache miss - call API
            Logger.debug('GeolocationService: Cache miss, calling reverse geocoding API');
            const addresses = await Location.reverseGeocodeAsync({
                latitude: coords.latitude,
                longitude: coords.longitude,
            });

            Logger.debug(`GeolocationService: Found ${addresses.length} addresses`);
            ...
            Logger.exception(error as Error, 'GeolocationService: Failed to reverse geocode');
            throw error;
        }
    }
    ...
    static async saveLocation(
        ...
        try {
            Logger.debug(`GeolocationService: Saving location "${name}"`);
            ...
            Logger.debug(`GeolocationService: Location "${name}" saved successfully`);
            return savedLocation;
        } catch (error) {
            Logger.exception(error as Error, `GeolocationService: Failed to save location "${name}"`);
            throw error;
        }
    }
    ...
    static async getSavedLocations(): Promise<SavedLocation[]> {
        try {
            const saved = await SecureStorage.getItem(SecureStorageKey.SAVED_LOCATIONS);
            if (!saved) {
                return [];
            }

            const locations: SavedLocation[] = JSON.parse(saved);
            Logger.debug(`GeolocationService: Retrieved ${locations.length} saved locations`);
            return locations;
        } catch (error) {
            Logger.exception(error as Error, 'GeolocationService: Failed to get saved locations');
            return [];
        }
    }
    ...
    static async getSavedLocationById(id: string): Promise<SavedLocation | null> {
        try {
            ...
            if (location) {
                Logger.debug(`GeolocationService: Found location with ID "${id}"`);
            } else {
                Logger.debug(`GeolocationService: No location found with ID "${id}"`);
            }

            return location || null;
        } catch (error) {
            Logger.exception(error as Error, `GeolocationService: Failed to get location by ID "${id}"`);
            return null;
        }
    }
     ...
     * @returns The saved location or null if not found
     */
    static async getSavedLocationByName(name: string): Promise<SavedLocation | null> {
        try {
            const locations = await this.getSavedLocations();
            const location = locations.find(loc => loc.name.toLowerCase() === name.toLowerCase());

            if (location) {
                Logger.debug(`GeolocationService: Found location with name "${name}"`);
            } else {
                Logger.debug(`GeolocationService: No location found with name "${name}"`);
            }

            return location || null;
        } catch (error) {
            Logger.exception(error as Error, `GeolocationService: Failed to get location by name "${name}"`);
            return null;
        }
    }
    ...
    static async updateSavedLocation(
        ...
        try {
            Logger.debug(`GeolocationService: Updating location with ID "${id}"`);

            const locations = await this.getSavedLocations();
            const index = locations.findIndex(loc => loc.id === id);

            if (index === -1) {
                Logger.debug(`GeolocationService: Location with ID "${id}" not found`);
                return null;
            }
            ...
            Logger.debug(`GeolocationService: Location updated successfully`);
            return locations[index];
        } catch (error) {
            Logger.exception(error as Error, `GeolocationService: Failed to update location with ID "${id}"`);
            throw error;
        }
    }
    ...
    static async deleteSavedLocation(id: string): Promise<boolean> {
        try {
            Logger.debug(`GeolocationService: Deleting location with ID "${id}"`);
            ...
            Logger.debug(`GeolocationService: Location deleted successfully`);
            return true;
        } catch (error) {
            Logger.exception(error as Error, `GeolocationService: Failed to delete location with ID "${id}"`);
            throw error;
        }
    }
    ...
    static async clearSavedLocations(): Promise<void> {
        try {
            ...
            Logger.debug('GeolocationService: All saved locations cleared');
        } catch (error) {
            Logger.exception(error as Error, 'GeolocationService: Failed to clear saved locations');
        }
    }
    ...
    static async findLocationsNearby(
        ...
        try {
            ...
            const nearby = locations.filter(location => {
                const distance = this.calculateDistance(coords, location.position.coords);
                return distance <= radiusMeters;
            });

            Logger.debug(`GeolocationService: Found ${nearby.length} locations within ${radiusMeters}m`);
            return nearby;
        } catch (error) {
            Logger.exception(error as Error, 'GeolocationService: Failed to find nearby locations');
            return [];
        }
    }
